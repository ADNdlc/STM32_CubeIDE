...
#else
    #define MAX_MEM_BANKS 1
#endif

/* Keil MDK / ARMCC编译器相关定义 */
#ifdef __CC_ARM
    #if !(__ARMCC_VERSION >= 6010050)   /* 使用ARMCC编译器时(Keil5及更早版本) */
...
const uint32_t memtblsize[SRAMBANK] = {
    MEM1_ALLOC_TABLE_SIZE, MEM2_ALLOC_TABLE_SIZE
};
/* 内存分块大小,记录对应内存块的大小 */
...
struct _m_mallco_dev mallco_dev = {
    ...
    {mem1base, mem2base}, /* 内存池(各个内存池数组的首地址,必须按顺序放) */
    {mem1mapbase, mem2mapbase},     /* 内存管理状态表(各个各个内存表的首地址) */
    {0, 0} /* 状态位,内存管理未就绪 */
};
#else
const uint32_t memtblsize[SRAMBANK] = {
...
struct _m_mallco_dev mallco_dev = {
    my_mem_init,    /* 内存初始化函数 */
    my_mem_perused, /* 内存使用率函数 */
    {mem1base}, /* 内存池(各个内存池数组的首地址,必须按顺序放) */
    {mem1mapbase},     /* 内存管理状态表(各个各个内存表的首地址) */
    {0} /* 状态位,内存管理未就绪 */
};
#endif
