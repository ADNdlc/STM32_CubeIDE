...
#include <string.h>
#ifdef USE_MEMPOOL
#define OLED_MEM_SOURCE SYS_MEM_INTERNAL
#endif
...
#ifdef USE_MEMPOOL
    oled_device_t *dev =
        (oled_device_t *)sys_malloc(OLED_MEM_SOURCE, sizeof(oled_device_t));
#else
    oled_device_t *dev =
        (oled_device_t *)malloc(sizeof(oled_device_t));
#endif
