...
// 定义一个映射结构体和数组来存储控件
#define MAX_UI_CONTROLS (MAX_DEVICES * MAX_PROPERTIES_PER_DEVICE)
typedef struct {
...
// 辅助函数，用于在创建UI时注册控件
void controller_register_ui_control(const char* deviceID, const char* propID, lv_obj_t* obj) {
    if (g_ui_map_count < MAX_UI_CONTROLS) {
        strcpy(g_ui_map[g_ui_map_count].deviceID, deviceID);
        strcpy(g_ui_map[g_ui_map_count].propID, propID);
...
lv_obj_t* controller_find_ui_control(const char* deviceID, const char* propID) {
    for (int i = 0; i < g_ui_map_count; i++) {
        if (strcmp(g_ui_map[i].deviceID, deviceID) == 0 && strcmp(g_ui_map[i].propID, propID) == 0) {
...
void ui_state_update_cb(const device_data_t* device, const char* prop_id) {
    ...
    // 获取最新的属性值
    const device_property_t* prop = DeviceManager_GetProperty(device->deviceID, prop_id);
    if (!prop) return;
