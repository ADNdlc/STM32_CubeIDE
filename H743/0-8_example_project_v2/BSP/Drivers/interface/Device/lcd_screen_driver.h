...
typedef struct {
  int lcd_init(lcd_driver_t *self, lcd_screen_info_t *info,
               uint8_t *buffer); // 设置信息和缓冲区并初始化

  int lcd_set_buffer(lcd_driver_t *self, uint8_t *buffer); // 重新设置缓冲区
  int lcd_set_dir(lcd_driver_t *self,
                  disp_direction_t direction); // 设置显示方向
  ...
  int lcd_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
               scr_color_t color); // 纯色填充
  int lcd_color_fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey,
                     scr_color_t *color); // 画面(数据)填充
} lcd_driver_ops_t;
