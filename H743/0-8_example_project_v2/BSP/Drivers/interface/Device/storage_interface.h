...
typedef struct {
    ...
    uint32_t erase_size;        /* 最小擦除单位 (Bytes)，NAND 为 Block Size, NOR 为 Sector Size */
    uint32_t write_size;        /* 最小写入单位 (Bytes)，NAND 为 Page Size, NOR 通常为 1 或 256 */
...
typedef struct {
    ...
    storage_status_t (*get_status)(storage_device_t *self); /* 存活检测/状态获取 */

    /**
     * @brief 数据操作 (字节偏移地址或块地址取决于文件系统适配)
     */
    int (*read)(storage_device_t *self, uint32_t addr, uint8_t *buf, size_t len);
    int (*write)(storage_device_t *self, uint32_t addr, const uint8_t *data, size_t len);
    int (*erase)(storage_device_t *self, uint32_t addr, size_t len); /* 擦除操作 */
