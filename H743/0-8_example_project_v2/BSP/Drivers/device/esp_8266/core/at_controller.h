...
// --- 命令执行结果 ---
typedef enum {
  AT_CMD_OK = 0,  // 结果为 "OK" or "SEND OK"
  AT_CMD_ERROR,   // 结果为 "ERROR" or "SEND FAIL"
  AT_CMD_TIMEOUT, // 超时
...
typedef struct at_controller_t {
  ...
  // 错误管理
  uint8_t error_count;          // Error 计数
  uint8_t consecutive_timeouts; // 连续超时计数(无响应)
  uint16_t busy_count;          // "busy p..."计数
} at_controller_t;
