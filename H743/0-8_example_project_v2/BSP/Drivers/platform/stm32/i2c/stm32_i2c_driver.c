                                    ...
                                    uint32_t timeout)
{
  ...
  HAL_StatusTypeDef status =
      HAL_I2C_Master_Receive(driver->config.resource.hi2c, dev_addr, buffer, size, timeout);
  return (status == HAL_OK) ? 0 : -1;
}
                               ...
                               uint8_t *data, uint16_t size, uint32_t timeout)
{
  ...
  HAL_StatusTypeDef status = HAL_I2C_Mem_Write(
		  driver->config.resource.hi2c, dev_addr, mem_addr, mem_addr_size, data, size, timeout);
  return (status == HAL_OK) ? 0 : -1;
}
                              ...
                              uint32_t timeout)
{
  ...
  HAL_StatusTypeDef status = HAL_I2C_Mem_Read(
		  driver->config.resource.hi2c, dev_addr, mem_addr, mem_addr_size, buffer, size, timeout);
  return (status == HAL_OK) ? 0 : -1;
}
#endif
