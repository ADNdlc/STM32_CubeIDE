...
#define SRAMDTCM                                                               \
  4 /* DTCM内存池,DTCM共128KB,此部分内存仅CPU和MDMA(通过AHBS)可以访问!!!! */
#define SRAMITCM                                                               \
  5 /* ITCM内存池,DTCM共64 KB,此部分内存仅CPU和MDMA(通过AHBS)可以访问!!!! */
...
// #define MEM1_MAX_SIZE 448 * 1024                            /* 最大管理内存
// 200K,H7的AXI内存总共512KB */
#define MEM1_MAX_SIZE 100 * 1024 // 384k = lvgl buffer , 2k = Heap+Stack
#define MEM1_ALLOC_TABLE_SIZE                                                  \
  MEM1_MAX_SIZE / MEM1_BLOCK_SIZE /* 内存表大小(内存池的一块对应列表的一项) */

/* mem2内存参数设定.mem2是外部的SDRAM内存 */
#define MEM2_BLOCK_SIZE 64 /* 内存块大小为64字节 */
#define MEM2_MAX_SIZE                                                          \
  27 * 1024 *                                                                  \
      1024 /* 最大管理内存28912K,外扩SDRAM总共32MB,LTDC占了2MB,还剩30MB. */
#define MEM2_ALLOC_TABLE_SIZE MEM2_MAX_SIZE / MEM2_BLOCK_SIZE /* 内存表大小 */

/* mem3内存参数设定.mem3是H7内部的SRAM1+SRAM2内存 */
#define MEM3_BLOCK_SIZE 64 /* 内存块大小为64字节 */
#define MEM3_MAX_SIZE                                                          \
  240 * 1024 /* 最大管理内存240K,H7的SRAM1+SRAM2共256KB(SRAM3的32K没用) */
...
struct _m_mallco_dev {
  ...
  uint8_t *membase[SRAMBANK];   /* 内存池
                                   管理SRAMBANK个区域的内存(各个内存池数组的首地址,必须按顺序放)
                                 */
...
void *myrealloc(uint8_t memx, void *ptr,
