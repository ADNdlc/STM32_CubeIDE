...
uint8_t g_norflash_mode = 1;         /* QSPI模式标志：0，SPI模式；1，QPI模式 */
...
void norflash_init(void) {
  ...
  if (g_norflash_type == W25Q256 ||
      g_norflash_type ==
          BY25Q256) /* SPI FLASH为25Q256, 必须使能4字节地址模式 */
  {
...
void norflash_qe_enable(void) {
   ...
   */
  qspi_send_cmd(FLASH_EnterQPIMode, 0,
                (0 << 6) | (0 << 4) | (0 << 2) | (1 << 0), 0);
...
void norflash_qspi_disable(void) {
   ...
   */
  qspi_send_cmd(FLASH_ExitQPIMode, 0, (0 << 6) | (0 << 4) | (0 << 2) | (3 << 0),
                0);
...
void norflash_write_enable(void) {
  if (g_norflash_mode) {
     ...
     */
    qspi_send_cmd(FLASH_WriteEnable, 0,
                  (0 << 6) | (0 << 4) | (0 << 2) | (3 << 0), 0);
  } else {
...
void norflash_write_disable(void) {
  if (g_norflash_mode) {
     ...
     */
    qspi_send_cmd(FLASH_WriteDisable, 0,
                  (0 << 6) | (0 << 4) | (0 << 2) | (3 << 0), 0);
  } else {
...
uint16_t norflash_read_id(void) {
  ...
  if (g_norflash_mode) {
     ...
     */
    qspi_send_cmd(FLASH_ManufactDeviceID, 0,
                  (3 << 6) | (2 << 4) | (3 << 2) | (3 << 0), 0);
  } else {
...
void norflash_read(uint8_t *pbuf, uint32_t addr, uint16_t datalen) {
   ...
   */
  qspi_send_cmd(FLASH_FastReadData, addr,
                (3 << 6) | (3 << 4) | (3 << 2) | (3 << 0), 8);
 ...
 * @retval      无
 */
static void norflash_write_page(uint8_t *pbuf, uint32_t addr,
                                uint16_t datalen) {
  norflash_write_enable(); /* 写使能 */
  /* QPI,页写指令,地址为WriteAddr,4线传输数据_32位地址_4线传输地址_4线传输指令,无空周期,NumByteToWrite个数据
   */
  qspi_send_cmd(FLASH_PageProgram, addr,
                (3 << 6) | (3 << 4) | (3 << 2) | (3 << 0), 0);
...
void norflash_write(uint8_t *pbuf, uint32_t addr, uint16_t datalen) {
  ...
  while (1) {
    ...
    if (i < secremain) /* 需要擦除 */
    {
      ...
      norflash_write_nocheck(norflash_buf, secpos * 4096,
                             4096); /* 写入整个扇区 */

    } else {
      norflash_write_nocheck(
          pbuf, addr, secremain); /* 写已经擦除了的,直接写入扇区剩余区间. */
    }
...
void norflash_erase_chip(void) {
   ...
   */
  qspi_send_cmd(FLASH_ChipErase, 0, (0 << 6) | (0 << 4) | (0 << 2) | (3 << 0),
                0);
...
void norflash_erase_sector(uint32_t saddr) {
   ...
   */
  qspi_send_cmd(FLASH_SectorErase, saddr,
                (0 << 6) | (3 << 4) | (3 << 2) | (3 << 0), 0);
