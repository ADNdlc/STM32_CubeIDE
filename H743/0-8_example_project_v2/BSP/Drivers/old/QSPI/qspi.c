...
uint8_t qspi_init(void) {
  ...
  g_qspi_handle.Init.ClockPrescaler =
      1; /* QPSI分频比，W25Q256最大频率为133M，
  ...
  g_qspi_handle.Init.SampleShifting = QSPI_SAMPLE_SHIFTING_HALFCYCLE; /* 采样移位半个周期(DDR模式下,必须设置为0)
                                                                       */
  g_qspi_handle.Init.FlashSize =
      POSITION_VAL(0X2000000) -
      1; /* SPI FLASH大小，W25Q256大小为32M字节,2^25，所以取权值25-1=24 */
  g_qspi_handle.Init.ChipSelectHighTime = QSPI_CS_HIGH_TIME_3_CYCLE; /* 片选高电平时间为3个时钟(10*3=30ns),即手册里面的tSHSL参数
                                                                      */
  g_qspi_handle.Init.ClockMode =
      QSPI_CLOCK_MODE_3;                        /* Mode3,空闲时CLK为高电平 */
...
void HAL_QSPI_MspInit(QSPI_HandleTypeDef *hqspi) {
  ...
  HAL_GPIO_Init(QSPI_BK1_NCS_GPIO_PORT,
                &gpio_init_struct); /* 初始化QSPI_BK1_NCS引脚 */

  gpio_init_struct.Pin = QSPI_BK1_IO2_GPIO_PIN;
  gpio_init_struct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(QSPI_BK1_IO2_GPIO_PORT,
                &gpio_init_struct); /* 初始化QSPI_BK1_IO2引脚 */

  gpio_init_struct.Pin = QSPI_BK1_IO3_GPIO_PIN;
  gpio_init_struct.Pull = GPIO_NOPULL;
  HAL_GPIO_Init(QSPI_BK1_IO3_GPIO_PORT,
                &gpio_init_struct); /* 初始化QSPI_BK1_IO3引脚 */
  ...
  HAL_GPIO_Init(QSPI_BK1_CLK_GPIO_PORT,
                &gpio_init_struct); /* 初始化QSPI_BK1_CLK引脚 */

  gpio_init_struct.Pin = QSPI_BK1_IO0_GPIO_PIN;
  HAL_GPIO_Init(QSPI_BK1_IO0_GPIO_PORT,
                &gpio_init_struct); /* 初始化QSPI_BK1_IO0引脚 */

  gpio_init_struct.Pin = QSPI_BK1_IO1_GPIO_PIN;
  HAL_GPIO_Init(QSPI_BK1_IO1_GPIO_PORT,
                &gpio_init_struct); /* 初始化QSPI_BK1_IO1引脚 */
}
 ...
 *   @arg       mode[1:0]: 指令模式; 00,无指令;  01,单线传输指令;
 * 10,双线传输指令; 11,四线传输指令.
 *   @arg       mode[3:2]: 地址模式; 00,无地址;  01,单线传输地址;
 * 10,双线传输地址; 11,四线传输地址.
 *   @arg       mode[5:4]: 地址长度; 00,8位地址; 01,16位地址;     10,24位地址;
 * 11,32位地址.
 *   @arg       mode[7:6]: 数据模式; 00,无数据;  01,单线传输数据;
 * 10,双线传输数据; 11,四线传输数据.
...
void qspi_send_cmd(uint8_t cmd, uint32_t addr, uint8_t mode, uint8_t dmcycle) {
  ...
  if (((mode >> 0) & 0x03) == 0)
    ...
    qspi_command_handle.InstructionMode =
        QSPI_INSTRUCTION_1_LINE; /* 指令模式 */
  else if (((mode >> 0) & 0x03) == 2)
    qspi_command_handle.InstructionMode =
        QSPI_INSTRUCTION_2_LINES; /* 指令模式 */
  else if (((mode >> 0) & 0x03) == 3)
    qspi_command_handle.InstructionMode =
        QSPI_INSTRUCTION_4_LINES; /* 指令模式 */
  ...
  qspi_command_handle.AlternateByteMode =
