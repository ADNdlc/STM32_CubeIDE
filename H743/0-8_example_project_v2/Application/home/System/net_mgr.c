...
static void wifi_event_handler(wifi_service_t *svc, wifi_status_t status, void *user_data) {
    log_i("WiFi raw status: %d (cur_mgr_state: %d)", status, g_state);

    if (status == WIFI_STATUS_GOT_IP) {
        if (g_state == NET_MGR_STATE_WAIT_IP || g_state == NET_MGR_STATE_RECONNECT_WAIT) {
        ...
        if (g_state == NET_MGR_STATE_WAIT_STABLE ||
            g_state == NET_MGR_STATE_READY ||
            g_state == NET_MGR_STATE_RUNNING) {
