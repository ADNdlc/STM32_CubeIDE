...
static void test_uart_queue_setup(void)
{
  ...
  // 初始化 UART 队列
  queue = uart_queue_create(test_usart, tx_buffer, TX_BUFFER_SIZE, rx_buffer, RX_BUFFER_SIZE);
}
...
static void test_uart_queue_loop(void)
{
  ...
  if (test_usart)
  {
    ...
    USART_TRANSMIT(test_usart, (const uint8_t *)start_msg, strlen(start_msg), 1000);
  }
...
static void test_uart_queue_teardown(void)
{
  ...
  USART_TRANSMIT(test_usart, (const uint8_t *)teardown_msg, strlen(teardown_msg), 1000);
}
...
static void test_uart_queue_batch_send(void)
{
  if (test_usart)
  {
    ...
    // 检查队列状态
    size_t remaining =
        uart_queue_tx_count(queue); // 不延时剩余72,延时1ms剩54
    char status_msg[50];
