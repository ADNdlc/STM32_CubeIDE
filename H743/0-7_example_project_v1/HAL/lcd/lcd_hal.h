...
// 内联多态调用函数
static inline void lcd_hal_draw_point(lcd_hal_t *self, uint16_t x, uint16_t y, uint32_t color) 
{
  self->vtable->base_vtable.draw_point(&self->base, x, y, color);
}

static inline uint32_t lcd_hal_read_point(lcd_hal_t *self, uint16_t x, uint16_t y) 
{
  return self->vtable->base_vtable.read_point(&self->base, x, y);
}

static inline void lcd_hal_fill_rect(lcd_hal_t *self, uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint32_t color) 
{
  self->vtable->base_vtable.fill_rect(&self->base, x, y, w, h, color);
}

static inline void lcd_hal_draw_bitmap(lcd_hal_t *self, uint16_t x, uint16_t y, uint16_t w, uint16_t h, const void *pSrc) 
{
  self->vtable->base_vtable.draw_bitmap(&self->base, x, y, w, h, pSrc);
}

static inline void lcd_hal_copy_buffer(lcd_hal_t *self, void *pDst, uint32_t xSize, uint32_t ySize, 
                                      uint32_t OffLineSrc, uint32_t OffLineDst, uint32_t PixelFormat) 
{
  self->vtable->base_vtable.copy_buffer(&self->base, pDst, xSize, ySize, OffLineSrc, OffLineDst, PixelFormat);
}

static inline void lcd_hal_set_orientation(lcd_hal_t *self, lcd_orientation_t orientation) 
{
