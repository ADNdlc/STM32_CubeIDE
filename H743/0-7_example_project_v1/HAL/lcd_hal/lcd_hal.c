...
// 实现基本绘图函数
void _lcd_hal_draw_point(lcd_driver_t *self, uint16_t x, uint16_t y,
                         uint32_t color) {
...
void _lcd_hal_draw_bitmap(lcd_driver_t *self, uint16_t x, uint16_t y,
...
void _lcd_hal_copy_buffer(lcd_driver_t *self, void *pDst, uint32_t xSize,
  ...
  if (self && self->ops && self->ops->copy_buffer) {
    self->ops->copy_buffer(self, pDst, xSize, ySize, OffLineSrc, OffLineDst,
                           PixelFormat);
  }
}

void _lcd_hal_set_orientation(lcd_driver_t *self,
...
void _lcd_hal_set_drawbuf(lcd_driver_t *self, uint16_t *buffer) {
  if (self && self->ops && self->ops->set_drawbuf) {
    LCD_SET_DRAWBUF(self, buffer);
  }
}

void _lcd_hal_set_displaybuf(lcd_driver_t *self, uint16_t *buffer) {
  if (self && self->ops && self->ops->set_displaybuf) {
    LCD_SET_DISPLAYBUF(self, buffer);
  }
}
...
lcd_hal_vtable_t lcd_hal_vtable = {
    .base_vtable = {
        ...
        .set_drawbuf = _lcd_hal_set_drawbuf,
        .set_displaybuf = _lcd_hal_set_displaybuf,
    }};
