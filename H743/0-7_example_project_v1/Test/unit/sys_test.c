...
static void memory_speed_test(uint8_t memx, const char *mem_name)
{
    ...
    log_i("Write Test: %d bytes | Time: %.3f ms | Speed: %.2f MB/s\r\n", TEST_DATA_SIZE, time_ms, speed_mbs);
    ...
    time_ms = (float)elapsed_cycles / (SystemCoreClock / 1000.0f);
    speed_mbs = (float)TEST_DATA_SIZE / 1024.0f / 1024.0f / (time_ms / 1000.0f);
    log_i("Read Test:  %d bytes | Time: %.3f ms | Speed: %.2f MB/s\r\n", TEST_DATA_SIZE, time_ms, speed_mbs);
...
void RamSpeed_test(void)
{
    ...
    // 这样测外部sdram的写入速度实际上测的是cpu写入FIFO的速度,
    // 并不是cpu发起传输到fmc完成传输操作的真实时间,会异常偏高，只有读取是正常的
...
void BaseRead_test(void)
{
    ...
    if (test_mem != NULL)
    {
        ...
        // 测试 realloc 扩大内存
        uint8_t *larger_mem =
            (uint8_t *)sys_realloc(SYS_MEM_INTERNAL, test_mem, 256);
        if (larger_mem != NULL)
        {
...
void advanced_sys_test(void)
{
    ...
    if (test_mem != NULL)
    {
        ...
        {
            log_i(
                "Address is in correct RAM_D1 region (0x24000000 - 0x2407FFFF)\r\n");
        }
        else
        {
        ...
        if (larger_mem != NULL)
        {
            ...
            {
                log_i("WARNING: Reallocated address is NOT in expected RAM_D1 "
                      "region!\r\n");
            }
    ...
    if (internal_mem != NULL)
    {
        ...
        {
            log_i("WARNING: SYS_MEM_INTERNAL address is NOT in expected RAM_D1 "
                  "region!\r\n");
        }
        sys_free(SYS_MEM_INTERNAL, internal_mem);
    }
    ...
    if (external_mem != NULL)
    {
        ...
        {
            log_i("WARNING: SYS_MEM_EXTERNAL address is NOT in expected SDRAM "
                  "region!\r\n");
        }
        sys_free(SYS_MEM_EXTERNAL, external_mem);
    }
    ...
    if (custom_mem != NULL)
    {
        ...
        {
            log_i("WARNING: SYS_MEM_CUSTOM address is NOT in expected DTCM "
                  "region!\r\n");
        }
        sys_free(SYS_MEM_CUSTOM, custom_mem);
    }
