...
static void key0_event_callback(gpio_key_t *key, KeyEvent event)
{
  ...
  case KeyEvent_SinglePress:
    ...
    thing_model_set_prop(config->device_id, "led0", (thing_value_t){.b = true}, THING_SOURCE_LOCAL);
    break;
  case KeyEvent_DoublePress:
    log_d("Hardware: key0 double pressed.");
    thing_model_set_prop(config->device_id, "led0", (thing_value_t){.b = false}, THING_SOURCE_LOCAL);
    break;
  case KeyEvent_TriplePress:
    log_d("Hardware: key0 triple pressed.");
    thing_model_set_prop(config->device_id, "led1", (thing_value_t){.b = true}, THING_SOURCE_LOCAL);
    break;
  case KeyEvent_LongPress:
    log_d("Hardware: key0 long pressed.");
    thing_model_set_prop(config->device_id, "led1", (thing_value_t){.b = false}, THING_SOURCE_LOCAL);
