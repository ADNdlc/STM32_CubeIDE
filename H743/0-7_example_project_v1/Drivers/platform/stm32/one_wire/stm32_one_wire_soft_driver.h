...
} stm32_one_wire_soft_config_t;

// STM32 软件模拟 One-Wire 驱动结构体
typedef struct {
  one_wire_driver_t base;              // 继承自 one_wire_driver_t 基类
  stm32_one_wire_soft_config_t config; // 配置
  uint8_t initialized;                 // 初始化标志
} stm32_one_wire_soft_driver_t;

/**
 * @brief 创建 STM32 软件模拟 One-Wire 驱动实例
...
/**
 * @brief 销毁 STM32 软件模拟 One-Wire 驱动实例
 * @param drv 驱动实例指针
 */
