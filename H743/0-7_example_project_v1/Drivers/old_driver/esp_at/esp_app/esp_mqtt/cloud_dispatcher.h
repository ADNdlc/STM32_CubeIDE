...
/**
 * @brief 解析并分发来自云端的命令(目前此函数只认功能点而不区分设备，因为云端模型目前将工程整体定义为一个设备)
...
/**
 * @brief 兼容旧版本，将云命令分发给DeviceManager进行处理
 * @param payload_json 命令的JSON字符串
 */
extern void device_manager_process_command(const char* payload_json);

#endif

/**
 * @brief 解析并分发来自云端的命令 (new)
 * @param deviceID       从MQTT Topic解析出的目标设备ID
 * @param payload_json   包含命令的JSON字符串
 */
