...
void norflash_init(void)
{
    ...
    log_i("NOR Flash initialized, ID: 0x%04X", g_norflash_type);
}
...
/**
 * @brief       擦除一个扇区
 * @note        注意,这里是字节地址,不是扇区编号!!
 *              擦除一个扇区的最少时间:150ms
 *
 * @param       saddr : 扇区地址(字节地址) 根据实际容量设置
 * @retval      无
 */
void norflash_erase_sector(uint32_t saddr)
{
    // 原函数将传入的地址再乘以4096，这是错误的
    // 如果传入的是字节地址，则不需要再乘以4096
    norflash_write_enable(); /* SET WEL */
    norflash_wait_busy();
    /* QPI,写扇区擦除指令,地址为saddr,无数据_32位地址_4线传输地址_4线传输指令,无空周期,0个字节数据 */
